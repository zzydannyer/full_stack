...
export const benchmark = new Elysia({ prefix: "/api/benchmark" })
  ...
  .get(
    "/json",
    ({ query }) => okResponse(createJsonResult(query.size)),
    {
      query: JsonQueryDto,
    },
  )
  .get(
    "/compute",
    ({ query }) => okResponse(createComputeResult(query.iterations)),
    {
  ...
  .get(
    "/database/read",
    async ({ query }) =>
      okResponse(await readBenchmark(query.database, query.limit)),
    {
  ...
  .post(
    ...
    async ({ query }) =>
      okResponse(
        await writeBenchmark(query.database, query.count, query.runId),
      ),
    {
  ...
  .delete(
    ...
    async ({ params, query }) =>
      okResponse(await deleteBenchmark(query.database, params.runId)),
    {
